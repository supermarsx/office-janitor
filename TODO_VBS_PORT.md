...
## Cross-Cutting Tasks
- [x] Build a complete CLI compatibility table for all VBS switches (C2R + MSI). Map each to Python semantics (e.g., `/QUIET` -> quiet logging, `/NOREBOOT` -> suppress reboot prompts, `/PREVIEW` -> dry-run). *Documented in `docs/CLI_COMPATIBILITY.md`.*
...
- [x] Logging parity: emit human + JSONL records that mirror OffScrub stages (stage0 detection, stage1 uninstall, rerun markers) and include legacy return-code semantics.
...
- [x] **OffScrubC2R.vbs**
  ...
  - [x] Port Click-to-Run scheduled task cleanup into data-driven cleanup steps; COM compatibility/registry residue cleanup added; ClickToRun cache/registration and CLSID entries purged.
- [x] **OffScrub_O16msi.vbs / OffScrub_O15msi.vbs**
...
- [x] **OffScrub10.vbs / OffScrub07.vbs / OffScrub03.vbs**
...
- [x] Document parity gaps and any intentionally unimplemented switches in `README.md`/module docstrings.
